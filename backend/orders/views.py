...
def clear_cart(request):
    ...
    if request.user.user_type != "customer":
        return Response(
            {"error": "Only customers can clear cart"}, status=status.HTTP_403_FORBIDDEN
        )
...
class FarmerOrderListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Order.objects.filter(items__farmer=farmer_profile)
            .distinct()
            .order_by("-created_at")
        )
...
def delete_order(request, order_number):
    ...
    if request.user.user_type != "admin":
        return Response(
            {"error": "Only admins can delete orders"}, status=status.HTTP_403_FORBIDDEN
        )
    ...
    if order.status not in ["pending", "confirmed"]:
        return Response(
            {"error": "Order cannot be deleted at this stage"},
            status=status.HTTP_400_BAD_REQUEST,
        )
